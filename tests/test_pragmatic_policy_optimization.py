...
def test_microbatches_are_count_weighted_without_optimizer_steps(monkeypatch):
    actor = ParameterActor()
    calls = []
    ...
    monkeypatch.setattr(ppo, "replay_joint_policy_gradient", replay)
...
def test_pilot_selects_only_changed_accepted_policy(
    monkeypatch, tmp_path, accepted, delta, selected,
):
    ...
    args = SimpleNamespace(checkpoint=tmp_path / "source.pt", output_dir=tmp_path / "run",
                           ...
                           development_pairs=16, microbatch=2, chunk_steps=2, learning_rate=1e-6)
