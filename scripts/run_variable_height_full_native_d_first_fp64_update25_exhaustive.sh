...
task_tmp="/home/mark/tmp/fly-variable-height-full-native-d-first-fp64-update25-exhaustive"
output_dir="$repo_root/runs/variable-height-hover/full-native-d-first-fp64-update25-exhaustive-audit-001"
mkdir -p "$task_tmp"
...
if [[ -e "$output_dir/report.json" ]]; then
  echo "final exhaustive-primary report already exists; refusing to retry" >&2
  exit 2
fi
...
exec_phase() {
  aira confine --memory-reserve 2G --memory-max 12G -- \
    .venv/bin/python \
      scripts/audit_variable_height_full_native_d_first_fp64_update25_exhaustive.py \
      "$@"
}
