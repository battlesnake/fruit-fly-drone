...
def _read_transmitters(path: Path) -> dict[int, str]:
    ...
    return {
        int(body): (label or "unclear").lower()
...
def build_hover_scaffold(raw_dir: Path, output: Path, config: BuildConfig) -> dict[str, Any]:
    ...
    for side in ("L", "R"):
        side_positions = np.asarray(
            [
                ...
                if annotations["somaSide"][row] == side
            ],
            dtype=np.int64,
        )
        points = np.asarray(
    ...
    attitude_mask = np.asarray(
        [
            ...
            for status, subclass in zip(
                annotations["status"], annotations["subclass"], strict=True
            )
        ]
    )
    ...
    for name in pool_names:
        pool_members.extend(
            graph_index[int(body)]
            for body in output_body_ids[name]
            if int(body) in graph_index
        )
    ...
    manifest: dict[str, Any] = {
        "format": "flydrone-hover-connectome-v1",
        "source": "MaleCNS v1.0",
        "config": {
