...
[`selected-magnitudes.json`](selected-magnitudes.json).

The original diagnostic retained only the selected treatment vector. Both stopped arm
vectors were subsequently regenerated by an exact rerun with the same seeds and protocol
and preserved in [`arm-magnitudes.json`](arm-magnitudes.json). The recovered treatment
differs from the original retained vector by at most `7.152557373046875e-07`; the recovery
file's SHA-256 is
`b8ae3d73147a7dd29500b67dc553bc0b29f08d8dc1e6ecd4ed4ee97c90784402`. This is a
deterministic reconstruction for read-only comparison, not a new selection or promoted
checkpoint.

Re-run the code introduced at commit `e8dd932` with:
